...
const registerUser = async(req,res)=>{
    ...
    return res.status(400).json("passwordbe a valid email");
...
const loginUser =async(req,res)=>{
    ...
    try{
        ...
        let user = await userModel.findOne({email});
        if(!user) return res.status(400).json("inavalid email/password");

        const isValidPassword = await bcrypt.compare(password,user.password);

        if(!isValidPassword) return res.status(400).json("inavalid email/password");
